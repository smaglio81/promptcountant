...
4. For each unresolved thread:
   ...
   e. **Resolve the thread** using the GraphQL mutation (see below). Every addressed thread — implemented or declined — MUST be resolved before moving on.
...
After requesting a review, Copilot typically takes 1–15 minutes. Poll using this approach:
...
4. If not, wait another 60 seconds and retry.
5. **Give up only after 15 attempts (15 minutes) with no new review.** At that point, tell the user that no new review arrived and ask them to check the PR manually.
